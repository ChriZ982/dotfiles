return {
    { -- full integration of git commands and UI
        ...
        config = function()
            ...
            keymap.set("n", "<leader>gf", "<cmd>Git fetch --prune --prune--tags<CR>", { desc = "Git fetch and prune (tpope/vim-fugitive)" })
            keymap.set("n", "<leader>gd", "<cmd>Git diff --ignore-all-space<CR>", { desc = "Git diff (tpope/vim-fugitive)" })
            keymap.set(
                "n",
                "<leader>gD",
                "<cmd>Git diff --ignore-all-space --cached<CR>",
    ...
    { -- show git diff in sign column and operate on hunks
        ...
        opts = {
            on_attach = function(bufnr)
                ...
                map("n", "<leader>hD", function() gs.diffthis("HEAD") end, { desc = "Diff with HEAD version (lewis6991/gitsigns.nvim)" })
                map("n", "<leader>hb", function() gs.blame_line({ full = true }) end, { desc = "Blame line (lewis6991/gitsigns.nvim)" })
            end,
        },
    },
}
